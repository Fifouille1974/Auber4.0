...
if (spip_version()>=3) {
	...
	include_spip('ecrire/inc/presenter_enfants');
		...
		while($row=sql_fetch($result)){
			...
			if (autoriser('voir','rubrique',$id_rubrique)){
				...
				$lib_bouton = (!acces_restreint_rubrique($id_rubrique) ? "" :
				  ...
				  generer_url_ecrire($exec,"id_rubrique=$id_rubrique") .
				  "'>".
				  typo($titre) .
				  "</a></span>";
	...
	function sous_enfant_rub($collection2){
		...
		while($row=sql_fetch($result3)){
			...
			if (autoriser('voir','rubrique',$id_rubrique2))
				$retour.="\n<li class='arial11 rubrique_12' dir='$lang_dir'><a href='" . generer_url_ecrire($exec,"id_rubrique=$id_rubrique2") . "'>".typo($titre2)."</a></li>\n";
		}
