...
function ciag_afficher_rubriques_auteur($id_auteur){
	...
	while ($row_admin = sql_fetch($result)) {
		...
		if ($ec) {
			$restreint_ec .= "\n<li style='padding:0;'><a href='$url_rubrique'>".typo($row_admin["multi"])."</a></li>";	
			if ($modif)
	...
	if (!$restreint AND !$restreint_grp) {
		...
		if ($restreint_grp) {
			$menu .= "<p class='actions'>"._T('ciag:grpauteurs_rubriques_auteur_info')."</p>"
		...
		if ($restreint_ec) {
			$menu .= "<p class='actions'>"._T('ciar:ec_auteur_info')."</p>"
		...
		if (defined('_DIR_PLUGIN_CIAG')) {
			...
			if ($grpauteurs) {
				...
				while ($row = sql_fetch($result))
					$groupes .= "\n<li style='padding:0;'><a href='".generer_url_ecrire('ciag_groupe_auteurs', "id_groupe=".$row['id_groupe'])."'>".typo($row['titre'])."</a></li>";
			}
		}
