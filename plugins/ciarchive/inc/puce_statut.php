...
if (spip_version()>=3){
	...
	function puce_statut_article_dist($id, $statut, $id_rubrique, $type='article', $ajax = false) {
		...
		$titles = array(
				  ...
				  "archive" => _T('ciarchive:texte_statut_archive'));
		...
		if ($ajax){
			...
			. "<span class='puce_article_popup' id='statutdecal$type$id' style='margin-left: -$clip"."px;'>"
			  ...
			  . afficher_script_statut($id, $type, -56, chemin('images/puce-archiver-8.png'), 'archive', $titles['archive'])
			  . "</span>";
		}
	...
	function puce_statut($statut, $atts='') {
		switch ($statut) {
			...
			case 'poubelle':
				...
				return http_img_pack($img, $alt, $atts);
			case 'archive':
				$img = chemin('images/puce-archiver-8.png');
