...
//--- debut ajout CI ---  
		$controle_spam = '';
		if (ciar_cispam()=='oui' AND $forum_stat=='publie' AND !$forum_id_auteur){
			$lien =  generer_url_ecrire('controle_forum', $args, true, true) . "#forum$id_forum";
			if (spip_version()>=3)
				$bouton_spam .= icone_verticale(_T('spam'), generer_action_auteur('instituer_forum',"$id_forum-spam", $lien),
				"forum-public-24.gif",
				"supprimer-sansdanger.gif", 'right', 'non');
			else
				$bouton_spam .= icone_inline(_T('spam'), generer_action_auteur('instituer_forum',"$id_forum-spam", $lien),
				"forum-public-24.gif",
				"supprimer-sansdanger.gif", 'right', 'non');
						
			$controle_spam = "<div style='float:".$GLOBALS['spip_lang_right'] ."; width: 80px; padding-bottom:20px;'>". $bouton_spam . "</div>";
		}
//--- fin ajout CI ---    
...
//----- Debut ajout CI -----
	// filtrer la requete en fonction des acces restreints
	if (spip_version()>=3) {
		// ne rien faire
	} else {
		...
		$where .= "(";
		$where .= "(F.id_rubrique>0 AND ".sql_in('F.id_rubrique', ciar_tableau_rubriques_exclues(), 'NOT').")";
		$where .= " OR (F.id_article>0 AND F.id_article NOT IN (".sql_get_select('zzza.id_article','spip_articles as zzza',sql_in('zzza.id_rubrique', ciar_tableau_rubriques_exclues(), ''))."))";
		$where .= " OR (F.id_breve>0 AND F.id_breve NOT IN (".sql_get_select('zzzb.id_breve','spip_breves as zzzb',sql_in('zzzb.id_rubrique', ciar_tableau_rubriques_exclues(), ''))."))";
		$where .= ")";
		

		if (defined('_DIR_PLUGIN_CIRR')){
			if ($GLOBALS['visiteur_session']['statut']=='0minirezo') {
				$ci_rubriques_auteur = array();
				$result = sql_select("id_rubrique", "spip_auteurs_rubriques", "id_auteur=".$GLOBALS['visiteur_session']['id_auteur']);
				while ($row = sql_fetch($result))
					$ci_rubriques_auteur[] = $row['id_rubrique'];
	
				if (count($ci_rubriques_auteur)>0) {
					if ($where)
						$where .= " AND ";	
					
					$where .= "(";
					$where .= "(F.id_rubrique>0 AND ".sql_in('F.id_rubrique', $ci_rubriques_auteur, '').")";
					$where .= " OR (F.id_article>0 AND F.id_article IN (".sql_get_select('zzza.id_article','spip_articles as zzza',sql_in('zzza.id_rubrique', $ci_rubriques_auteur, ''))."))";
					$where .= " OR (F.id_breve>0 AND F.id_breve IN (".sql_get_select('zzzb.id_breve','spip_breves as zzzb',sql_in('zzzb.id_rubrique', $ci_rubriques_auteur, ''))."))";
					$where .= ")";
				}
			}
		}
...
//--- debut ajout CI ---  
function ciar_cispam(){
	static $return;

	if (!$return) {
		if (defined('_DIR_PLUGIN_CISPAM')){
