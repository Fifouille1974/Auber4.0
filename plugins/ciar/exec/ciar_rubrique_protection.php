...
function exec_ciar_rubrique_protection_dist()
{
	exec_ciar_rubrique_protection_args(intval(_request('id_rubrique')));
}

// http://doc.spip.org/@exec_rubriques_edit_args
function exec_ciar_rubrique_protection_args($id_rubrique)
{
...
//	echo pipeline('affiche_gauche',array('args'=>array('exec'=>'ciar_rubrique_protection','id_rubrique'=>$id_rubrique),'data'=>''));
...
//	echo debut_droite('', true);
	...
	if (spip_version()>=3){
		$exec = "rubrique";
		$icone_retour = icone_verticale(_T('icone_retour'), generer_url_ecrire($exec,"id_rubrique=$id_rubrique"), $ze_logo, "rien.gif",$GLOBALS['spip_lang_left']);
	} else {
		$exec = "naviguer";
		$icone_retour = icone_inline(_T('icone_retour'), generer_url_ecrire($exec,"id_rubrique=$id_rubrique"), $ze_logo, "rien.gif",$GLOBALS['spip_lang_left']);
	}
	...
	echo recuperer_fond("prive/editer/ciar_rubrique_protection", $contexte);
